...
export const amazon = {
  ...
  getListings: (searchQuery, resultsLimit, currency, language, priceFrom, priceTo, customerReviewsRating) =>
    getListings(amazon.timeMultiplier, searchQuery, resultsLimit, currency, language, priceFrom, priceTo, customerReviewsRating),

  /**
   * Get listing info from Amazon
   * @async
   * @param {String} link - listing ling;
   * @param {String} currency - currency code. You can use both "text" or "code" from `getParams().currencies`;
   * @param {String} language - interface language code. You can use both "text" or "code" from `getParams().currencies`;
   * @param {Number} reviewsLimit - parameter defines the reviews amount you want to get. Must be a number or `Infinity`. Default - 10;
   * @return {Array.<Object>} - an array with listings.
   */
  getListingInfo: (link, currency, language, reviewsLimit) => getListingInfo(amazon.timeMultiplier, link, currency, language, reviewsLimit),
